...
// Each MCP session (one per connected client) gets its own Server instance.
...
async function runServer() {
  ...
  if (mode === 'http') {
    const { port } = await startHttpServer({
      ...
      port: Number(process.env.MCP_PORT) || 3939,
      sessionTtlMs: Number(process.env.MCP_SESSION_TTL_MS) || 0,
      idleTimeoutMs: Number(process.env.MCP_IDLE_TIMEOUT_MS) || 0
    });
    // Node ignores SIGTERM by default when running as PID 1 in a container,
    // so handle it explicitly to make `docker stop` graceful and fast.
    for (const sig of ['SIGINT', 'SIGTERM']) {
      process.once(sig, () => process.exit(0));
    }
    console.error(`WinDirStat MCP Server running on http port ${port} (/mcp)`);
    return;
  }
