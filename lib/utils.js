...
export function categorizeItem(name, tier1Keywords, tier2Keywords) {
  ...
  const lowerTier1Keywords = tier1Keywords.map(kw => kw.toLowerCase());
  const lowerTier2Keywords = tier2Keywords.map(kw => kw.toLowerCase());

  // For tier1: match exact name, or endsWith for extension-like keywords (starting with .)
  if (lowerTier1Keywords.some(kw => {
    if (kw.startsWith('.')) {
