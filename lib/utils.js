...
// Async so long scans yield to the event loop: in HTTP mode one process
// serves every client, and a synchronous walk would stall all of them.
export async function getFolderSize(dirPath, currentDepth = 0, maxDepth = 3) {
  let totalSize = 0;
  let fileCount = 0;
      ...
      try {
        if (item.isDirectory()) {
          if (currentDepth < maxDepth) {
            const sub = await getFolderSize(fullPath, currentDepth + 1, maxDepth);
